<template>
  <div class="seventytwo">
    ...
    <p style="margin-left: -500px;margin-top: 100px;font-size: 30px">隔</p>
...
<script>
// 寄件人表单
import SenderForm from './seventyTwo/SenderForm'
// 预约信息
import ReservationForm from './seventyTwo/ReservationForm'
// 收件人表单
import ReceiveForm from './seventyTwo/ReceiveForm'
// 下单界面
import PlaceOrder from './seventyTwo/PlaceOrder'
export default {
  name: 'SeventyTwo',
  data () {
    return {
      active: 0,
      index: 0,
      refreshCache: ['SenderForm', 'ReceiveForm'], // 是否刷新缓存
        ...
        if (this.active < 4) {
          ...
          if (this.active > 3) {
            ...
            setTimeout(() => {
              loading.close()
              // 异步实现刷新组件缓存
              this.refreshCache = ['SenderForm', 'ReceiveForm']
            }, 1000)
            this.index = 0
            this.active = 0
            this.refreshCache = []
